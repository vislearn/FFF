from fff.data.qm9 import bond_analyze

try:
    from rdkit import Chem
    from fff.data.qm9.rdkit_functions import BasicMolecularMetrics
    use_rdkit = True
...
def check_stability(positions, atom_type, dataset_info, debug=False):
    ...
    for i in range(len(x)):
        for j in range(i + 1, len(x)):
            ...
            elif dataset_info['name'] == 'mol-grid':
                order = 0
            else:
                raise ValueError("Unknown dataset %s" % dataset_info['name'])
            nr_bonds[i] += order
            nr_bonds[j] += order
    nr_stable_bonds = 0
...
def analyze_stability_for_molecules(molecule_list, dataset_info, individual=False):
    ...
    for mol in processed_list:
        ...
        is_stable.append(validity_results[0])
